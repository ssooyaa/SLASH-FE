...
const RequestManagerTop = ({contractId}) => {
  ...
  const handleMonthlyDataChange = async (selectedYear, selectedMonth, contractId) => {
    const data = await getMonthlyData(selectedYear, selectedMonth, contractId);
    setMonthlyData(data.data);
  };

  // 초기 데이터 요청
  useEffect(() => {
      handleMonthlyDataChange(year, month, contractId); // 초기 데이터 한 번 호출
  }, [year, month, contractId]);
