...
const RequestManagerTop = () => {
    ...
    const REGISTERED = monthlyData?.statusCountList
        ? monthlyData.statusCountList.find(item => item.name === "REGISTERED")?.count ?? 0
        : 0;
    const IN_PROGRESS = monthlyData?.statusCountList
        ? monthlyData.statusCountList.find(item => item.name === "IN_PROGRESS")?.count ?? 0
        : 0;
    const COMPLETED = monthlyData?.statusCountList
        ? monthlyData.statusCountList.find(item => item.name === "COMPLETED")?.count ?? 0
        : 0;
